...
class Division extends DV_Controller {
	...
	function __construct()
	{
		...
		$this->set_dv(TRUE); # For testing without divisions - see MY_Controller.php
	...
	public function index()
	{
		...
		switch($dv_url = $this->division_builder->get_dv_url())
		{
			...
			$this->load->library('division/division_config'); # so now we can put divsion configs
			...
			if( ! empty($this->division_builder->layout))
			{
				...
				foreach($this->division_builder->layout as $position_key => $subpositions)
				{
					foreach($subpositions as $subposition_key => $params)
					{
						$this->division_builder->layout[$position_key][$subposition_key]['view'] = 
						$surfaces_content[$this->division_builder->layout[$position_key][$subposition_key]['alias_key']]['view'];
					}
				}
